...
def voronoi(points_set: set[Point]):
    ...
    events = Tas()
    events.tas = [Event(p, SITE_EVENT) for p in points_set]
    assert events.est_tas()
    ...
    while not events.est_vide():

        # 3. do Remove the event with largest y-coordinate from Q.
        event: Event = events.extraire_maximum()
        ...
        if event.is_site_event():
            ...
            handle_site_event(pi, events, bleach_line, dcel)
            ...
            gamma = event.circle
            handle_circle_event(gamma, events, bleach_line, dcel)
...
def handle_circle_event(gamma, events: Tas, bleach_line: Arbre, dcel: DCEL):
