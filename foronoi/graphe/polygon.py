from foronoi.graphe import Coordonee, Vertex, HalfEdge
...
class Polygon(Subject):
    def __init__(self, tuples):
        ...
        super().__init__()
        points = [Coordonee(x, y) for x, y in tuples]
        self.points = points
        ...
        center = Coordonee((max_x + min_x) / 2, (max_y + min_y) / 2)
    ...
    def _order_points(self, points):
        clockwise = sorted(points, key=lambda point: (-180 - Geometrie.calculer_angle(point, self.center)) % 360)
        return clockwise
    ...
    def _get_ordered_vertices(self, vertices):
        ...
        clockwise = sorted(vertices,
                           key=lambda vertex: (-180 - Geometrie.calculer_angle(vertex, self.center)) % 360)
        return clockwise

    @staticmethod
    def _get_closest_point(position, points):
        distances = [Geometrie.distance(position, p) for p in points]
    ...
    def finish_polygon(self, edges, existing_vertices, points):
        ...
        for index in range(0, len(vertices) - 1):
            ...
            # Récupérer origine
            origine = vertices[index]
            end = vertices[index + 1]

            # If vertex is connected to other edges, update the cell
            if len(origine.connected_edges) > 0:
                cell = origine.connected_edges[0].jumelle.point_incident

            # Create the edge
            edge = HalfEdge(cell, origine=origine, jumelle=HalfEdge(None, origine=end))
            origine.connected_edges.append(edge)
            end.connected_edges.append(edge.jumelle)

            # Add first edge if needed
            if cell:
                cell.arete_entree = cell.arete_entree or edge

            # Connect edges
            if len(end.connected_edges) > 0:
                edge.set_suiv(end.connected_edges[0])

            # Connect to incoming edge, or previous edge
            if len(origine.connected_edges) > 0:
                origine.connected_edges[0].jumelle.set_suiv(edge)
            elif previous_edge is not None:
                previous_edge.set_suiv(edge)
    ...
    def finish_edges(self, edges, **kwargs):
        ...
        for edge in edges:
            ...
            if edge.get_origine() is None or not self.inside(edge.get_origine()):
                self._finish_edge(edge)

            if edge.jumelle.get_origine() is None or not self.inside(edge.jumelle.get_origine()):
                self._finish_edge(edge.jumelle)

            if edge.get_origine() is not None and edge.jumelle.get_origine() is not None:
                resulting_edges.append(edge)
            else:
                edge.supprimer()
                edge.jumelle.supprimer()
                self.notify_observers(Message.DEBUG, payload=f"Edges {edge} and {edge.jumelle} supprimées!")
    ...
    def _finish_edge(self, edge):
        ...
        start = edge.get_origine(y=sweep_line, max_y=self.max_y)

        # End should be a vertex
        end = edge.jumelle.get_origine(y=sweep_line, max_y=self.max_y)
    ...
    def _get_intersection_point(self, orig, end):
        ...
        for i in range(0, len(p) - 1):
            intersection_point = Geometrie.trouver_intersection(orig, end, p[i], p[i + 1])
        ...
        max_distance = Geometrie.distance(orig, end)

        # Find the intersection point that is furthest away from the start
        if points:
            distances = [Geometrie.distance(orig, p) for p in points]
