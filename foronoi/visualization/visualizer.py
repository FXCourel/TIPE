...
class Visualizer:
    ...
    def plot_vertices(self, vertices=None, **kwargs):
        ...
        Parameters
        ----------
        vertices: list(:class:`foronoi.graphe.Vertex`), optional
    ...
    def plot_outgoing_edges(self, vertices=None, scale=0.5, **kwargs):
        ...
        for vertex in vertices:
            for edge in vertex.connected_edges:
                ...
                new_end = Coordonee(start.xd + direction[0] * scale, start.yd + direction[1] * scale)
    ...
    def plot_sites(self, points=None, show_labels=True, color=Colors.CELL_POINTS, zorder=10):
        ...
        Parameters
        ----------
        points: list(:class:`foronoi.graphe.Point`), optional
            The vertices to display. By default, the `voronoi`'s vertices will be used.
        show_labels: bool
            Display the labels of the cell points, of format "`P#`", where `#` is the `n`th point from top to bottom.
        color: str
        ...
        # Scatter points
        self.canvas.scatter(xs, ys, s=50, marker=".", color=color, zorder=zorder)

        # Add descriptions
        if show_labels:
            for point in points:
                self.canvas.text(point.xd, point.yd, s=f"P{point.nom if point.nom is not None else ''}", zorder=15)
    ...
    def plot_edges(self, edges=None, sweep_line=None, show_labels=True, color=Colors.EDGE, **kwargs):
        ...
        Parameters
        ----------
        edges: list(:class:`foronoi.graphe.HalfEdge`), optional
        ...
        for edge in edges:
            ...
            self._plot_edge(edge.jumelle, sweep_line, print_name=False, color=color)
    ...
    def plot_border_to_site(self, edges=None, sweep_line=None):
        ...
        for edge in edges:
            ...
            self._draw_line_from_edge_midpoint_to_incident_point(edge.jumelle, sweep_line)
    ...
    def _plot_arc_labels(self, x, plot_lines, bottom, sweep_line, arcs):
        ...
        for index in unique_indices:
            ...
            self.canvas.text(x_mean, y, s=f"{arcs[index].origin.nom}", size=12, color=Colors.VALID_CIRCLE, zorder=15)
    ...
    def _origins(self, edge, sweep_line=None):
        ...
        start = edge.get_origine(sweep_line, max_y)
        end = edge.jumelle.get_origine(sweep_line, max_y)
